...
(require (only-in "command.rkt" CMD* command%))
(require (only-in "stack.rkt" stack-init))
(define (assert v p) (unless (p v) (error 'assert)) v)
(define defn-command
   (new
    command%
    (id 'define)
...
(define (forth-eval* lines)
   (for/fold
    ((e (cons defn-command CMD*)) (s (stack-init)))
    ((ln (in-list lines)))
...
(define (forth-eval E S token*)
   (match
    (for/or ((c (in-list E))) ((get-field exec c) E S token*))
    ('EXIT (values #f S))
