(module zo-string typed/racket/base/no-check
   (#%module-begin
    ...
    (define g371 string?)
    (define g372
      (letrec ((g21179364 (recursive-contract g21179364368 #:chaperone))
               (g21179365 (recursive-contract g21179365369 #:chaperone))
               (g21179366 (recursive-contract g21179366370 #:chaperone))
               (g21179364368
                (cons/c
                 g371
                 (listof (cons/c g371 (-> (values (or/c g371 g21179364)))))))
               (g21179365369
                (cons/c
                 g371
                 (listof (cons/c g371 (-> (values (or/c g371 g21179365)))))))
               (g21179366370
                (cons/c
                 g371
                 (listof (cons/c g371 (-> (values (or/c g371 g21179366))))))))
        g21179365))
    (define g373 '#t)
    (define g374 '#f)
    ...
    (module require/contracts racket/base
      ...
      (define g363 string?)
      (define l/1 (-> g362 (values g363)))
      (provide g362 g363 l/1 g360 g361 (contract-out (srcloc->string l/1))))
    (require (prefix-in -: (only-in 'require/contracts srcloc->string))
             (except-in 'require/contracts srcloc->string))
    ...
    (provide Spec)
    (provide (contract-out (zo->spec generated-contract358))
             (contract-out (zo->string generated-contract359)))))
