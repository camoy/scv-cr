...
(define-values
  (g964 g965 g966 g967 g968 g969 g970 generated-contract957)
  (let ()
    (local-require
     racket/contract
     racket/class
    ...
    (letrec ((g964 (lambda (x) (zo? x)))
             ...
             (g969 '#f)
             (g970 (or/c g968 g969))
             (generated-contract957 (-> g964 g965 (values g967 g970))))
      (values g964 g965 g966 g967 g968 g969 g970 generated-contract957))))
...
(define (def-syntaxes-> z field-name)
   (match
    ...
    ("rhs"
     (match (def-syntaxes-rhs z) ((or (? expr? rhs) (? seq? rhs)) rhs) (_ #f)))
...
(define (install-value-> z field-name)
   (match
    ...
    ("body" (match (install-value-body z) ((? expr-or-seq? body) body) (_ #f)))
    (_ #f)))
...
(define (apply-values-> z field-name)
   (match
    field-name
    ("proc" (match (apply-values-proc z) ((? expr-or-seq? proc) proc) (_ #f)))
    ("args-expr"
     (match
...
(define (with-immed-mark-> z field-name)
   (match
    field-name
    ("key" (match (with-immed-mark-key z) ((? expr-or-seq? proc) proc) (_ #f)))
    ("def-val"
     (match
