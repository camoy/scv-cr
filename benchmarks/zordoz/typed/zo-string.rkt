...
(define-values
  (g394 g398 g399 generated-contract385 g400 g401 g402 generated-contract386)
  (let ()
    (local-require
     racket/contract
     racket/class
    ...
    (letrec ((g394 (lambda (x) (zo? x)))
             (g398 string?)
             (g399
              (letrec ((g10788391
                        (recursive-contract g10788391395 #:chaperone))
                       (g10788392 (recursive-contract g10788392396 #:flat))
                       (g10788393 (recursive-contract g10788393397 #:flat))
                       (g10788391395
                        (cons/c
                         g398
                         (listof
                          (cons/c g398 (-> (values (or/c g398 g10788391)))))))
                       (g10788392396
                        (cons/c
                         g398
                         (listof
                          (cons/c g398 (-> (values (or/c g398 g10788392)))))))
                       (g10788393397
                        (cons/c
                         g398
                         (listof
                          (cons/c g398 (-> (values (or/c g398 g10788393))))))))
                g10788392))
             (generated-contract385 (-> g394 (values g399)))
             (g400 '#t)
             (g401 '#f)
             (g402 (or/c g400 g401))
             (generated-contract386 (->* (g394) (#:deep? g402) (values g398))))
      (values
       g394
       g398
       g399
       generated-contract385
       g400
       g401
       g402
       generated-contract386))))
(require (only-in racket/contract contract-out))
(provide (contract-out (zo->spec generated-contract385))
          (contract-out (zo->string generated-contract386)))
(provide Spec)
...
(define (zo->string z #:deep? (deep? #t)) (format-spec deep? (zo->spec z)))
...
(define (mod->spec z)
   ...
   (:
    unexported->spec
    (->
     (Listof (List Exact-Nonnegative-Integer (Listof Symbol) (Listof Symbol)))
     String))
   ...
   (:
    ...
    (->
     (Listof
      (Pair Exact-Positive-Integer (Listof (U def-syntaxes seq-for-syntax))))
     String))
...
(define (module-shift->spec ms)
   (list
    ...
    (lcons
     "from"
     (cond ((module-shift-from ms) => module-path-index->string) (else "#f")))
    (lcons
     "to"
...
(define (decoded-module-binding->spec dmb)
   (list
    ...
    (lcons
     "nominal-path"
     (cond
      ((decoded-module-binding-nominal-path dmb) => module-path-index->string)
      (else "#f")))
    (lcons
     "nominal-export-name"
...
(define (list->string f xs)
   (format
    "[~a]"
    (format-list #:sep " " (for/list : (Listof String) ((x : A xs)) (f x)))))
(: listof-form-or-any->string (-> (Listof (U form Any)) String))
(define (listof-form-or-any->string xs) (list->string form-or-any->string xs))
...
(define (pad str w #:char (c #\space))
   ...
   (cond ((< l w) (format "~a~a" str (make-string (- w l) c))) (else str)))
