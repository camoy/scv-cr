(module zo-transition racket/base
   (#%module-begin
    ...
    (define (compilation-top-> z field-name)
      (match
       ...
       ("code" (define res (compilation-top-code z)) (if (form? res) res #f))
       (_ #f)))
    (define (prefix-> z field-name)
      ...
      (match
       ...
       ("stxs" (for/list ((sx (prefix-stxs z)) #:when sx) sx))
       (_ #f)))
    ...
    (define (wrap-> z field-name)
      (match
       ...
       ("multi-scopes" (for/list ((ms (wrap-multi-scopes z))) (car ms)))
       (_ #f)))
    ...
    (define (multi-scope-> z field-name)
      (match
       field-name
       ("scopes" (for/list ((mss (multi-scope-scopes z))) (cadr mss)))
       (_ #f)))
